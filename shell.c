...
int main() {
  ...
  char command_backup[MAX_INPUT_LENGTH]; // Kept for potential future use
  ...
  while (true) {
    do { 
        ...
        if ((fgets(user_input_buffer, MAX_INPUT_LENGTH, stdin) == NULL) && ferror(stdin)) {
            fprintf(stderr, "Error reading input");
            exit(0);
        }

    } while(user_input_buffer[0] == 0x0A);  // Skip if just ENTER pressed
    ...
    char* final_argument = command_arguments[argument_index-1];
    ...
    if (strcmp(command_arguments[0], "cd") == 0) {
      ...
      for (int echo_index = 1; command_arguments[echo_index] != NULL; echo_index++) {
        if (command_arguments[echo_index][0] == '$') {
          char* env_value = getenv(command_arguments[echo_index] + 1);
          if (env_value != NULL) {
            printf("%s ", env_value);
          } else {
      ...
      if (command_arguments[1] != NULL) {
        char* env_value = getenv(command_arguments[1]);
        if (env_value != NULL) {
          printf("%s\n", env_value);
        } else {
 ...
 */
void execute_command_with_pipes_and_redirection(char* command_arguments[]) {
  // Check for I/O redirection before handling pipes
  for (int arg_index = 0; command_arguments[arg_index] != NULL; arg_index++) {
    // Output redirection
    if (strcmp(command_arguments[arg_index], ">") == 0 && command_arguments[arg_index+1] != NULL) {
      int output_fd = open(command_arguments[arg_index+1], O_WRONLY | O_CREAT | O_TRUNC, 0644);
      if (output_fd < 0) {
        perror("open");
        return;
      }
    ...
    // Input redirection
    if (strcmp(command_arguments[arg_index], "<") == 0 && command_arguments[arg_index+1] != NULL) {
      int input_fd = open(command_arguments[arg_index+1], O_RDONLY);
      if (input_fd < 0) {
        perror("open");
        return;
      }
  ...
  int num_pipes = pipe_command_count;
  pipe_command_count++; // Total commands = number of pipes + 1
  
  if (num_pipes == 0) {
    // Simple command with no pipes
    execvp(commands_by_pipe[0][0], commands_by_pipe[0]);
    // If we get here, execvp failed
    perror("execvp");
  }
  else {
    // Create pipes
    int pipe_file_descriptors[num_pipes][2];
    ...
    int process_ids[pipe_command_count];
    for (int cmd_index = 0; cmd_index < pipe_command_count; cmd_index++) {
      ...
      if (process_ids[cmd_index] == 0) {
        ...
        // Execute the command
        execvp(commands_by_pipe[cmd_index][0], commands_by_pipe[cmd_index]);
        // If we get here, execvp failed
        perror("execvp");
        exit(1);
      }
    ...
    for (int proc_index = 0; proc_index < pipe_command_count; proc_index++) {
